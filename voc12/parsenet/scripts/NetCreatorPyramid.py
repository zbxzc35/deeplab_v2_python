...
def deeplab_vgg16(proto_path, train, data_root, source, num_labels, batch_size=20, prefix=None, source_id=None):
    ...
    n.fc7_norm = BN(n.drop7)

    size = 41 if train else 65
    n.pool7 = L.Pooling(
        n.drop7, 
        pool=P.Pooling.AVE,
        kernel_size=size, 
    ...
    n.pool7_interp = L.Interp(
        n.pool7,
        height=size,
        width=size
    )
    n.pool7_norm = BN(n.pool7_interp)
    n.pool7_drop = L.Dropout(n.pool7_norm, dropout_ratio=0.3, in_place=True)

    n.fc7_score = L.Convolution(
        n.fc7_norm, 
    ...
    n.score = L.Eltwise(n.pool7_score, n.fc7_score, operation=P.Eltwise.SUM)
